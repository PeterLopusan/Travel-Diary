...
import com.peterlopusan.traveldiary.models.user.CreateUserBody
import com.peterlopusan.traveldiary.models.user.UserInfo
...
class AuthViewModel : ViewModel() {
    ...
    fun login(email: String, password: String): MutableLiveData<Boolean> {
        ...
        auth.signInWithEmailAndPassword(email, password).addOnCompleteListener { task ->
            ...
            task.exception?.message?.let { showToast(TranslateApiManager().translateErrorMessage(it)) }
            getInfoAboutMe()
        }
        return isSuccessful
    }
    ...
    fun createAccount(countryCode: String): MutableLiveData<Boolean> {
        ...
        auth.createUserWithEmailAndPassword(createBody.email, createBody.password).addOnCompleteListener { task ->
            ...
            showLogs(it.message)
            showToast(TranslateApiManager().translateErrorMessage(it.message))
        }
    ...
    fun deleteAccount(actionAfterDone: () -> Unit) {
        val uid = auth.uid!!
